...
public class LeafNode<K, V> implements Node<K, V>, ComplexPage {
	...
	class KeyStruct {
		...
		private KeyStruct(int position) {
		...
		public V getValue() {
			byte[] bytes = new byte[valueSerializer.getSerializedLength()];
	...
	public class LeafNodeIterator implements Iterator<V> {
		...
		public V next() {
			...
			V result = currentKeyStruct.getValue();
	...
	private KeyStruct firstKeyStructEqualOrLargerThan(K from) {
		KeyStruct current = new KeyStruct(0);
		do {
